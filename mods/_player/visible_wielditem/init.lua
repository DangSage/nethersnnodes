...
-- Model attachments and item tweaks
visible_wielditem.model_attachments = {
    default = {
        bonename = "Arm_Right",
        position = vector.new(0.025, 0.35, 0.25),
...
function visible_wielditem.get_attachment(modelname, itemname)
    ...
    local rotation = modlib.quaternion.from_euler_rotation_deg(attachment.rotation)
    ...
    local function apply_tweaks(tweaks)
        ...
        if tweaks.rotation then
            rotation = quaternion_compose(rotation, modlib.quaternion.from_euler_rotation_deg(tweaks.rotation))
        end
    ...
    attachment.rotation = modlib.quaternion.to_euler_rotation_irrlicht(rotation)
    return attachment
end
...
minetest.register_entity(entity_name, {
    ...
    _remove = function(self)
        self.on_deactivate = modlib.func.no_op
        self.object:remove()
        entities[self._player:get_player_name()] = nil
    end
})

-- Wield item change handler
